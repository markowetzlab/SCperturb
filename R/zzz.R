...
.onLoad <- function(libname, pkgname) {
    ...
    for (url in needsdl_data) {
        localfile <- file.path(.libPaths(), libname, "data", url)
        if (any(file.exists(localfile))) {
            lf <- localfile[[which(file.exists(localfile))[[1]]]]
            utils::download.file(file.path(base_url, "data", url), lf)
        }
    }
    for (idx in 1:length(needsdl_dr)) {
        localfile <- file.path(.libPaths(), libname, "data-raw", needsdl_subdir[[idx]], needsdl_dr[[idx]])
        if (any(file.exists(localfile))) {
            lf <- localfile[[which(file.exists(localfile))[[1]]]]
            utils::download.file(url, lf)
        }
